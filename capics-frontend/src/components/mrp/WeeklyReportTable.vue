        ...
        <table class="main-table">
          ...
          <tbody>
            <tr v-for="row in report" :key="row.itemNumber">
              <td class="sticky-col sticky-col-1" :title="row.itemNumber">{{ row.itemNumber }}</td>
              <td class="sticky-col sticky-col-2" :title="row.description">{{ row.description }}</td>
              <td
                v-for="(col, idx) in columns"
                :key="'data-' + idx"
...
// Compute column groups from columns data
...
/* Header sticky columns - highest z-index */
