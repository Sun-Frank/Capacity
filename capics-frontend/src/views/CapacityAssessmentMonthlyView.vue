<template>
  ...
  <div class="page">
    ...
    <div class="page-header">
      ...
      <p class="page-subtitle">静态产能核算（月）模块</p>
              ...
              <tr v-for="item in selectedLineData" :key="item.itemNumber + '_' + item.componentNumber">
                ...
                <template v-for="(month, idx) in months" :key="'data-' + idx">
                  ...
                  <td class="data-cell">
                    ...
                    {{ formatDemand(item[month + '_demand']) }}

                  </td>

                  <td class="data-cell">

                    {{ formatLoading(item[month + '_loading']) }}
...
// sessionStorage keys
const SESSION_KEY = 'capics_capacity_static_monthly'
...
const loadCapacityAssessmentMonthly = async () => {
